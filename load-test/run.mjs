...
if (!["memory", "redis"].includes(profile)) {
  console.error("Usage: node load-test/run.mjs --profile memory|redis [--quick] [--skip-build] [--keep-stack]");
  process.exit(1);
}
...
try {
  ...
  for (const scenario of selectedScenarios) {
    if (scenario.warmupSec) {
      ...
      await runAutocannon({
        ...
        scenario: { ...scenario, durationSec: scenario.warmupSec, connections: Math.min(10, scenario.connections) },
      });
      await sleep(2_000);
    }
  ...
  await writeReport({
    ...
    profile,
    suite: "benchmark",
    meta: { startedAt, finishedAt, appUrl, metricsUrl, quick, resourceLimits: "2 vCPU / 4 GiB (app)" },
    results,
    metricsBefore,
    metricsAfter,
  });
