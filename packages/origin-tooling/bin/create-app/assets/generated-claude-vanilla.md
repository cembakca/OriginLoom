...
`@originloom/shared`, `@originloom/core` and `@originloom/vanilla` and is **consumed,
never copied**. This app owns its route table, its product contract, its pages and
its chrome.

This app uses **no UI framework**: pages are functions returning HTML built with
the `html` tagged template (which escapes interpolated values), and islands are
plain modules that receive a DOM element. The views are registered once via
`createHtmlRenderer` in `server/product/renderer.ts`.
...
3. App → platform imports use `@originloom/core/*`, `@originloom/vanilla/*` and
