...
| Cache purge ve inspect uçları          | operations portu, `server/index.ts`                          | [cache-purge.md](./cache-purge.md)                       |
| Sağlayıcıya yönlendirme (PRG + oturum) | `server/api/referrals.ts`                                    | [referrals.md](./referrals.md)                           |
| Araç sayfası (SSR + island + API)      | `/calculator`, `src/islands/loan-calculator.tsx`             | [tools.md](./tools.md)                                   |
| Editoryal içerik, Article + FAQPage    | `/guides`, `src/lib/metadata/jsonld-article.ts`              | [seo.md](./seo.md)                                       |
| CMS menüsü (header/hamburger/footer)   | `server/services/menu.ts`, `src/lib/menu.ts`                 | [caching.md](./caching.md)                               |
| Filtre, sıralama, sayfalama            | `/catalog`, `src/lib/catalog-query.ts`                       | [lists.md](./lists.md)                                   |
| Route param doğrulama (şekil/varlık)   | `server/services/route-domains.ts`, `/catalog/:category`     | [route-params.md](./route-params.md)                     |
| Gateway kimliği (tracking/IP/device)   | `server/services/*.ts`, `tests/gateway-identity.test.ts`     | [caching.md](./caching.md)                               |
...
| Responsive media ve ikon üretimi       | `server/media.config.json`, `src/assets/`, `public/*-icons/` | `pnpm media`, `pnpm icons`                               |
