...
`@originloom/vanilla`. This app is **consumed, never copied**.

This app has **no UI framework**: pages are functions returning HTML built with
the `html` tagged template, and islands are plain modules that receive a DOM
element.
...
Imported as `@originloom/core/*`, `@originloom/vanilla/*` and `@originloom/shared/*`.
The core is framework-free: it renders through the `OriginRenderer` contract in
`@originloom/shared/render`, which `@originloom/vanilla/server` implements with
plain strings.
...
| Product contract given to the platform                | `server/product/runtime.ts` (`OriginRuntime`), `renderer.ts` (views), `document-shell.ts` (metadata policy), `boundary-pages.ts` |
...
3. **Import boundaries:** the app reaches the platform through `@originloom/core/*`,
   `@originloom/vanilla/*` and `@originloom/shared/*`. Inside the app use the `~/`
   (→ `src`) and `@server/` (→ `server`) aliases. The platform layering is
   `{core, vanilla} → shared`: the renderer package and the server core never
   import each other.
